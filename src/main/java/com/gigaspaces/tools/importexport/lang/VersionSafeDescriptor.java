...
public class VersionSafeDescriptor implements Serializable {
	...
	public static VersionSafeDescriptor create(SpaceTypeDescriptor typeDescriptor,
		...
		final boolean isInative;

		final String superTypeName;
		if (typeDescriptor instanceof ITypeDesc) {
			// InactiveTypeDesc.getSuperTypeName fails
			ITypeDesc typeDesc = (ITypeDesc) typeDescriptor;
			String[] superClassNames = typeDesc.getSuperClassesNames();
			if (superClassNames == null || superClassNames.length == 0) {
				superTypeName = null;
			} else {
				// super classes start from current class up to Object
				superTypeName = superClassNames[1];
			}

			isInative = typeDesc.isInactive();
		} else {
    ...
    public SpaceTypeDescriptor toSpaceTypeDescriptor() {
		if (isInactive) {
			...
			while (currentTypeName != null) {
				superTypeNames.add(this.getTypeName());
				Class<?> superClass;
				try {
					superClass = Class.forName(this.getTypeName()).getSuperclass();
				...
				if (superClass == null) {
					...
					currentTypeName = superClass.getName();
				}
			}

			return new InactiveTypeDesc(this.getTypeName(), superTypeNames.toArray(new String[0]));
		}
